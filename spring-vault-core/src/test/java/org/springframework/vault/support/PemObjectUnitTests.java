...
class PemObjectUnitTests {
	...
	void setUp() {
		assertThat(this.privateDir).exists()
			.isDirectoryContaining(file -> "localhost.public.key.pem".equalsIgnoreCase(file.getName()));
		assertThat(this.privateDir).exists()
			.isDirectoryContaining(file -> "localhost.decrypted.key.pem".equalsIgnoreCase(file.getName()));
	}
